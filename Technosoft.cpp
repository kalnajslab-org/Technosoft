...
bool Technosoft::Sync(void) {
	...
	for (int i = 0; i < 15; i++) {
		...
		Serial.println(ret_byte,HEX);

		if (ret_byte == SYNC_BYTE) 
		{
			Serial.println("Controller Synced");
			return true;
		}
	}
...
bool Technosoft::SetAxisOn(void) {
	...
	if ((write_size = Serialize_Packet(&request, request_buffer)) == 0) return false;
	err_code = Perform_Transaction(request_buffer, write_size);
	if (!err_code) {
		//try twice
		err_code = Perform_Transaction(request_buffer, write_size);
		if (err_code) return true;
		
		Serial.print("Set Axis on Fault: ");
...
bool Technosoft::SendEndInit(void) {
	...
	if ((write_size = Serialize_Packet(&request, request_buffer)) == 0)
	{
		 Serial.println("Serialize Packet Failed, write size = ");
